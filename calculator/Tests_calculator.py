...
class TestCalculator(unittest.TestCase):
    ...
    def test_Vector_evaluate_expression(self):
        ...
        self.assertEqual(evaluate_expression('{1;2;3} ^ 2'), '{1;4;9}')
        self.assertEqual(evaluate_expression('{1;2;3} ^ {2;3;2}'), '{1;8;9}')
        self.assertEqual(evaluate_expression('{1;2;3} / {1;1;1}'), '{1;2;3}')
        self.assertEqual(evaluate_expression('{9;9;9} / {3;2;9}'), '{3;4;1}')
        self.assertEqual(evaluate_expression('{9;9;9} / 8'), '{1;1;1}')
    ...
    def test_Vector_Error(self):
        ...
        self.assertEqual(evaluate_expression('12 / {1;2;3}'), 'Ошибка. Невозможно разделить число на вектор')
        self.assertEqual(evaluate_expression('{-1;-2;-3} ^ {3;1;0}'), '{-1;-2;1}')
        self.assertEqual(evaluate_expression('{-1;-2;-3} / {3;1;1}'), '{0;-2;-3}')
        self.assertEqual(evaluate_expression('{9;9;9} / {3;2;0}'), 'Ошибка. Деление на 0. Вектор {3;2;0}')
        self.assertEqual(evaluate_expression('{9;9;9} / 0'), 'Ошибка. Нельзя разделить вектор на 0.')
